  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">新闻类型</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">媒体发布渠道</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">新闻通稿撰写与公关媒体传播指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 新闻稿撰写与公关发稿专家');
...
const metricsList = [
  {
    "key": "journalisticStandardRigor",
    "label": "新闻专业倒金字塔规范"
  },
  {
    "key": "fiveWOneHPrecision",
    "label": "5W1H 要素精准度"
  },
  {
    "key": "prMediaDistributability",
    "label": "媒体发稿传播力"
  },
  {
    "key": "crisisCommunicationDiscretion",
    "label": "危机公关分寸得体度"
  },
  {
    "key": "brandValueConveyance",
    "label": "品牌价值传递分"
  }
];
const guideRules = [
  {
    "title": "导语第一段涵盖 5W1H",
    "advice": "新闻稿第一段（导语）必须交代时间、地点、人物、事件、原因及结果，让编辑 3 秒了解全貌",
    "avoid": "切忌第一段大篇幅抒情而没有新闻事实"
  },
  {
    "title": "客观中立严禁广告词",
    "advice": "用第三人称客观叙述（如“据悉”、“该公司宣布”），避免使用“超级棒”、“太惊艳了”等主观感叹词",
    "avoid": "避免把新闻通稿写成硬广促销单"
  },
  {
    "title": "严查广告法违规词汇",
    "advice": "规避“最”、“第一”、“顶级”等新广告法禁用极限词，使用可考证的客观数据替代",
    "avoid": "严禁使用虚假引人误导的夸大表达导致合规风险"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[XINWEN_SCORES\][\s\S]*?\[\/XINWEN_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[XINWEN_SCORES\](.*?)\[\/XINWEN_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('xinwen_history_records');
...
const saveHistory = () => {
  localStorage.setItem('xinwen_history_records', JSON.stringify(historyList.value));
};
